...
def sync_study(self, cgds_study_pk: int, only_failed: bool):
    ...
    if cgds_study.attempt >= 3:
        logging.warning(f'CGDSStudy {cgds_study.pk} has reached attempts limit.')
        cgds_study.state = CGDSStudySynchronizationState.REACHED_ATTEMPTS_LIMIT
        cgds_study.save(update_fields=['state'])
        return

    # Increments the attempt and sets the state of the CGDSStudy to IN_PROCESS
    cgds_study.attempt += 1
    cgds_study.state = CGDSStudySynchronizationState.IN_PROCESS
    cgds_study.save(update_fields=['attempt', 'state'])
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(mode='wb') as out_file:
            ...
            extract_file_and_sync_datasets(cgds_study, downloaded_path, only_failed)
