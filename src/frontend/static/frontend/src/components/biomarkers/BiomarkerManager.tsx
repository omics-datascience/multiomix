...
import ky from 'ky'
import { getDjangoHeader, alertGeneralError, getFileTypeSelectOptions, getDefaultNewTag, copyObject, getInputFileCSVColumns } from '../../utils/util_functions'
 ...
 * Component's state
 */
interface BiomarkerManagerState {
    tags: DjangoTag[],
 ...
 */
interface BiomarkerManagerProps {
    handleChangeConfirmModalState: (setOption: boolean, headerText: string, contentText: string, onConfirm: () => void) => void
    onTagsChange?: (tags: DjangoTag[]) => void
}

class BiomarkerManager extends React.Component<BiomarkerManagerProps, BiomarkerManagerState> {
     ...
     */
    getUserInstitutions () {
        ky.get(urlUserInstitutions, { signal: this.abortController.signal })
            .then((response) => {
                response.json<DjangoInstitution[]>()
    ...
    getUserTags () {
        ...
        const searchParams = { type: TagType.FILE }
    ...
    uploadFile = () => {
        ...
        this.setState({ uploadingFile: true }, () => {
            if (this.isEditing()) {
                ...
                }).then((response) => this.uploadSuccess(response as UploadResponse))
                    .catch((err) => {
...
export { NewFile, BiomarkerManager }
